...
function simulateButtons() {
  ...
  requestAnimationFrame(simulateButtons);
}
...
function displayNode(nodeId) {
  ...
  typeWriterSmooth(paragraph, node.text, 50, function() {
    // テキスト表示完了後、選択肢ボタンを生成
    // まず、既存の選択肢ボタンをすべて削除（シーン切替時のクリーンアップ）
    const choicesContainer = document.getElementById("choices");
    choicesContainer.innerHTML = "";
    movingButtons = []; // 物理シミュレーション用の配列もクリア
    ...
    if (node.choices.length > 0) {
      node.choices.forEach(choice => {
        ...
        // 初期位置：コンテナ内で、ボタンサイズはまだ決まっていないので暫定的に0～(containerSize - 50)とする
      ...
      // シミュレーション開始（requestAnimationFrame でループ）
      requestAnimationFrame(simulateButtons);
    } else {
      ...
      // 再スタートボタンも物理シミュレーションの対象にする場合は同様の処理を行う
      const contRect = document.getElementById("game-container").getBoundingClientRect();
      let initX = Math.random() * (contRect.width - 50);
      let initY = Math.random() * (contRect.height - 50);
